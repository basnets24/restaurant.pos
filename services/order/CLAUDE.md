...
- `Services/` — `CartService`, `FinalOrderService` (implements `IOrderService`), `DinerOrderService` (the customer-facing flow behind `DinerController`), `DiningTableService`, `PricingService` (singleton — stateless pricing math), `NotificationService`, `CurrentUserAccessor`, `CustomerOrderHistoryService` and `CustomerNotificationService` (see Gotchas), `AbandonedOrderSweeper` (hosted service, see Gotchas), `Catalog/CatalogMenuClient` and `Tenancy/TenantDirectoryClient` (the service's only two synchronous cross-service calls — catalog for modifier prices, identity for restaurant display names)
- `Entities/` — `Cart`, `Order`, `DiningTable`, `Notification` (EF entities, tenant-scoped via `Common.Library`'s `TenantEfRepository<T>`, registered in `Program.cs`), plus `CustomerOrderSummary` and `CustomerNotification`, which are **not** tenant-scoped and have no repository (see Gotchas)
...
- **Two notification subsystems, not one.** `Notification` + `NotificationService` address restaurant **staff**: tenant-scoped, persisted, and pushed live to the SignalR floor group. `CustomerNotification` + `CustomerNotificationService` address the **diner**: not tenant-scoped (their orders span restaurants), poll-only (a diner is in no tenant group and must not be put in one), and read through `/diner/notifications`. The type constants are separate sets too (`NotificationType` vs `CustomerNotificationType`) — nothing should be valid in both. Customer notifications are raised from five places: `InventoryReservedConsumer` (confirmed), `InventoryReserveFaultedConsumer` (rejected), `PaymentFailedConsumer`, and `FinalOrderService.MarkPaidAsync`/`CancelAsync`. `NotifyAsync` no-ops for an order with no `CustomerId`, so the shared staff paths can call it unconditionally.
- `IOrderService.CancelAsync` takes a `reason` that becomes the **diner-facing** body of their cancellation notification — the sweep sets it so a timed-out order doesn't read as the restaurant refusing the order. Don't put staff-worded text (or `msg.Reason` off an event) in it.
- Don't format currency server-side. The frontend's `money()` owns the app's one currency assumption; a `:C` in a message string renders in the server host's culture, which is not the diner's.
