// src/services/menu.items.ts
import { http } from "../lib/http";
import { MenuAPI } from "../config/api.menu";

export interface MenuItemDto {
    id: string; name: string; description: string; price: number;
    category: string; isAvailable: boolean; createdAt: string;
}
...
export const MenuItems = {
    list: async (q?: Parameters<typeof MenuAPI.list>[0]): Promise<PageResult<MenuItemDto>> => {
    ...
    setAvailability: async (id: string, value: boolean): Promise<void> => {
        await http.post(MenuAPI.availability(id), value, { headers: { "Content-Type": "application/json" } });
    },
};
