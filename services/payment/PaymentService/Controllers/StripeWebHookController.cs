...
public class StripeWebhookController : ControllerBase
{
    ...
    try
    {
        ...
        switch (stripeEvent.Type)
        {
            ...
            // Optional: if you also store PaymentIntentId in ProviderRef, you can handle these:
            case "payment_intent.payment_failed":
            {
                var pi = (Stripe.PaymentIntent)stripeEvent.Data.Object!;
                var payment = await _repo.GetAsync(p => p.Provider == "Stripe" && p.ProviderRef == pi.Id);
